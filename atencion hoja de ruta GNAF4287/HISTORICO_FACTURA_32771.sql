...
-- HITORICO seguimiento_capacitacion.facturas_persona_inscripcion
	...
	SELECT(
	    ...
	    from(
			select * from seguimiento_capacitacion.facturas_persona_inscripcion f where f.perpre_codigo IN ('20181018_ACZBDL' )
		    ) as t
	) as traza,
...
-- HITORICO seguimiento_capacitacion.facturas
	...
	SELECT(
		select array_to_json (array_agg (row_to_json (t)))
	    from(
			select * from seguimiento_capacitacion.facturas where fac_codigo = 438063
		    ) as t
	) as traza,
	'seguimiento_capacitacion.facturas' as tabla,
	now() as fecha_ejecucion, 
	'Gabriel Casas M.' as funcionario_sgsir_responsable, 
	'factura  32771 - actualizar el saldo' as comentario_accion_realizada;
...
-- HITORICO seguimiento_capacitacion.personas_inscripcion
	...
	SELECT(
		select array_to_json (array_agg (row_to_json (t)))
	    from(
			SELECT * FROM seguimiento_capacitacion.personas_inscripcion pe WHERE pe.perpre_codigo IN ( '20181018_ACZBDL' )
		    ) as t
	) as traza,
...
-- HITORICO seguimiento_capacitacion.certificacion_persona
	...
	SELECT(
		select array_to_json (array_agg (row_to_json (t)))
	    from(
			select * from seguimiento_capacitacion.certificacion_persona where perpre_codigo = '20181018_ACZBDL'
		    ) as t
	) as traza,
...
-- HITORICO seguimiento_capacitacion.evaluacion_docente_personas_inscritas
	...
	SELECT(
		select array_to_json (array_agg (row_to_json (t)))
	    from(
			select * from seguimiento_capacitacion.evaluacion_docente_personas_inscritas where perpre_codigo='20181018_ACZBDL'
		    ) as t
	) as traza,
...
-- HITORICO acceso_externo.cuenta_persona_inscripcion
	...
	SELECT(
		select array_to_json (array_agg (row_to_json (t)))
	    from(
			SELECT * from acceso_externo.cuenta_persona_inscripcion cpi WHERE cpi.perpre_codigo IN ( '20181018_ACZBDL' )
		    ) as t
	) as traza,
...
-- HITORICO acceso_externo.persona_preinscripcion
	...
	SELECT(
		select array_to_json (array_agg (row_to_json (t)))
	    from(
			select * from acceso_externo.persona_preinscripcion pe WHERE pe.perpre_codigo IN   ('20181018_ACZBDL') and pe.perpre_estado = 3
		    ) as t
	) as traza,
	'acceso_externo.persona_preinscripcion' as tabla,
	now() as fecha_ejecucion, 
	'Gabriel Casas M.' as funcionario_sgsir_responsable, 
	'factura  32771 - actualizar el estado de la inscripcion eliminada' as comentario_accion_realizada;

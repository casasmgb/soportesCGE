...
where pcc.procurcod_sigla = 'CE/TR-CS03-929/2018' 
*/
DO $$
DECLARE 
...
BEGIN
	...
	WHERE edpi.evadocperins_estado = 2 AND edpi.procur_codigo = 1640 AND edpi.asidoc_codigo = 1;
	GET DIAGNOSTICS _actualizados = ROW_COUNT;	
	-- si no es lo esperado , hacer un rollback
	IF _actualizados != 61	THEN   
		RAISE EXCEPTION transaction_rollback;
	END IF ; 
	raise notice '% Filas actualizadas.', _actualizados;
END
$$;
